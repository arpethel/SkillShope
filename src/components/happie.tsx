...
export function Happie() {
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch("/api/happie", {
        ...
        body: JSON.stringify({ message: userMsg, context: { currentPage: pathname } }),
      });
  ...
  return (
    <>
      {/* Floating toggle button — 44px min touch target */}
      {!open && (
        <button
          onClick={() => setOpen(true)}
          className="fixed bottom-6 right-6 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[var(--accent)] shadow-lg active:scale-95 transition-transform"
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-end justify-end">
          ...
          <div
            className="absolute inset-0 bg-black/40 backdrop-blur-sm md:bg-transparent md:backdrop-blur-none md:pointer-events-none"
            onClick={() => setOpen(false)}
          />

          {/* Chat panel */}
          <div
            className="relative flex w-full flex-col bg-[var(--bg)] border-t border-[var(--border)] md:m-6 md:h-[600px] md:w-[420px] md:rounded-2xl md:border md:shadow-2xl"
            style={{
          ...
          >
            ...
            <div className="flex shrink-0 items-center justify-between border-b border-[var(--border)] px-4 py-3">
              ...
              <button
                onClick={() => setOpen(false)}
                className="flex h-9 w-9 items-center justify-center rounded-lg text-[var(--text-secondary)] hover:bg-[var(--bg-secondary)] hover:text-[var(--text)] transition-colors"
                style={{ WebkitTapHighlightColor: "transparent" }}
              >
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain px-4 py-4">
                  ...
                  <div className="w-full space-y-2">
                    {[
                      ...
                      <button
                        ...
                        className="block w-full rounded-lg border border-[var(--border)] px-3 py-3 text-left text-xs text-[var(--text-secondary)] hover:border-[var(--accent)]/40 hover:text-[var(--text)] active:bg-[var(--bg-secondary)] transition-colors"
                        style={{ WebkitTapHighlightColor: "transparent" }}
                      >
                        {suggestion}
                      </button>
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`mb-3 ${msg.role === "user" ? "text-right" : "text-left"}`}>
                  <div
                    className={`inline-block max-w-[88%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                      msg.role === "user"
                ...
                <div className="rounded-lg border border-[var(--green)]/30 bg-[var(--green)]/5 p-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-[var(--accent)] py-2.5 text-xs font-medium text-white hover:bg-[var(--accent-hover)] active:opacity-80 transition-colors"
                      style={{ WebkitTapHighlightColor: "transparent" }}
                    >
                    ...
                    <button
                      ...
                      className="rounded-lg border border-[var(--border)] px-3 py-2.5 text-xs text-[var(--text-secondary)] hover:text-[var(--text)] transition-colors"
                    >
                      Dismiss
                    </button>
                  </div>
            ...
            <div className="shrink-0 border-t border-[var(--border)] px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[var(--accent)] text-white transition-colors hover:bg-[var(--accent-hover)] active:opacity-80 disabled:opacity-40"
                  style={{ WebkitTapHighlightColor: "transparent" }}
                >
